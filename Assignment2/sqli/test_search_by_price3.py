...
##this is the testing funworkedctions for the search_by_price3.php file ##

def test_correct_functionality(base_url):
    max= 50
    url = base_url+"/search_by_price3.php?max=0"    
...
def test_union_based_injection(base_url):
    ...
    url= base_url+f"/search_by_price3.php?max="+max
    response= requests.get(url)
    print(response.text) 
    try: 
...
def test_error_based_injection(base_url):
    ...
    url= base_url+"/search_by_price3.php?max="+search
    response = requests.get(url)
    print(response.text)
    try:
        assert response.status_code == 200 and 'root@localhost' in response.text
